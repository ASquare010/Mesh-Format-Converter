...
def mesh_convert(file_obj, output_format, mesh_type):
    ...
    command = [
        ...
        "--background",
        "--python",
        "/vizcom/app/apps/mesh_convert/src/blender_convert.py",
        "--",
        temp_input,
        temp_output,
        mesh_arg
...
def gradio():
    iface = gr.Interface(
        fn=mesh_convert,
        inputs=[
            gr.components.File(label="Upload 3D Mesh File (ZIP allowed)"),
        ...
        outputs=gr.components.File(label="Download Converted File"),
